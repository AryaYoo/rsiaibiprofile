    ...
    <section id="detail-lowongan" class="section-padding" style="background: white;">
        <div class="container">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 48px; align-items: start;"
                class="career-detail-grid">
                {{-- Left Content: Details --}}
                <div class="reveal">
                    {{-- Deskripsi --}}
                    <div style="margin-bottom: 40px;">
                        <h2
                            style="font-size: 1.6rem; color: var(--primary); margin-bottom: 16px; border-left: 4px solid var(--primary-light); padding-left: 12px;">
                            Deskripsi Pekerjaan</h2>
                        <p
                            style="color: var(--text-muted); line-height: 1.8; font-size: 0.98rem; text-align: justify; margin: 0;">
                    ...
                    @if($career->day_to_day_tasks)
                        <div style="margin-bottom: 40px;">
                            <h2
                                style="font-size: 1.6rem; color: var(--primary); margin-bottom: 16px; border-left: 4px solid var(--primary-light); padding-left: 12px;">
                                Tugas & Tanggung Jawab (Jobdesc)</h2>
                    ...
                    {{-- Security Warning --}}
                    <div
                        style="background: #FFF9E6; border: 1px solid #FFE0B2; border-radius: 12px; padding: 24px; margin-top: 48px; display: flex; gap: 16px;">
                        <div style="color: #FF9800; font-size: 1.8rem; line-height: 1; flex-shrink: 0;">
                            <i class="bi bi-exclamation-triangle-fill"></i>
                        </div>
                        <div>
                            <h4 style="color: #E65100; margin-top: 0; margin-bottom: 8px; font-size: 1.05rem;">Peringatan
                                Keamanan</h4>
                            <p style="color: #5D4037; font-size: 0.9rem; line-height: 1.6; margin: 0; text-align: justify;">
                                RSIA IBI Surabaya <strong>tidak pernah memungut biaya apapun</strong> (termasuk biaya
                                transportasi, akomodasi, biaya tes, atau biaya administrasi lainnya) selama proses seleksi
                                penerimaan karyawan baru. Mohon berhati-hati terhadap pihak yang mengatasnamakan RSIA IBI
                                Surabaya dan menawarkan bantuan dengan imbalan finansial.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="reveal" style="position: sticky; top: 100px;">
                    <div
                        style="background: var(--bg-main); border-radius: var(--radius); padding: 32px; border: 1px solid var(--border-soft); box-shadow: var(--shadow-sm);">
                        <h3 style="font-size: 1.3rem; color: var(--primary); margin-bottom: 24px; font-weight: 700;">
                            Informasi Pekerjaan</h3>

                        <div style="display: flex; flex-direction: column; gap: 20px; margin-bottom: 32px;">
                            <div style="display: flex; align-items: flex-start; gap: 12px;">
                                <i class="bi bi-geo-alt"
                                    style="color: var(--primary-light); font-size: 1.2rem; margin-top: 2px;"></i>
                                <div>
                                    <div
                                        style="font-size: 0.8rem; color: var(--text-muted); font-weight: 600; text-transform: uppercase;">
                                        Penempatan</div>
                                    <div style="font-weight: 700; color: var(--text-main); font-size: 0.95rem;">
                                        {{ $career->placement }}</div>
                                </div>
                            </div>
                            ...
                            @if($career->salary_min !== null || $career->salary_max !== null)
                                <div style="display: flex; align-items: flex-start; gap: 12px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-weight: 700; color: var(--text-main); font-size: 0.95rem;">
                                            @if($career->salary_min !== null && $career->salary_max !== null)
                                                Rp {{ number_format($career->salary_min, 0, ',', '.') }} - Rp
                            ...
                            @if($career->contact_email || $career->contact_whatsapp)
                                <div style="padding-top: 16px; border-top: 1px dashed var(--border-soft); margin-top: 8px;">
                                    <div
                                        style="font-size: 0.75rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 10px;">
                                        Kontak Information</div>
                                    @if($career->contact_email)
                                        <div
                                            style="display: flex; align-items: center; gap: 8px; font-size: 0.88rem; margin-bottom: 6px;">
                                            <i class="bi bi-envelope-at" style="color: var(--primary-light);"></i>
                                            <a href="mailto:{{ $career->contact_email }}"
                                                style="color: var(--primary); text-decoration: underline;">{{ $career->contact_email }}</a>
                                        </div>
                                    @endif
                        ...
                        @if($hasLink && $isEmailType)
                            ...
                            @endphp
                            <a href="{{ $mailtoUrl }}" class="btn btn-accent"
                                style="width: 100%; justify-content: center; height: 48px; font-size: 0.95rem; border-radius: 10px;">
                                <i class="bi bi-envelope-fill mr-2"></i> Lamar via Email
                            </a>
                        @elseif($hasLink)
                            {{-- Kondisi 2: Google Form / Link --}}
                            <a href="{{ str_starts_with($career->apply_link, 'http') ? $career->apply_link : 'https://' . $career->apply_link }}"
                        ...
                        @else
                            ...
                            <div
                                style="padding: 16px; border: 1px dashed #10B981; border-radius: 12px; background: #F0FDF4; text-align: center;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 8px;">
                                    @if($career->contact_email)
                                        ...
                                        @endphp
                                        <a href="{{ $cMailtoUrl }}" class="btn"
                                            style="width: 100%; justify-content: center; height: 38px; font-size: 0.85rem; border-radius: 8px; background: #059669; color: white;">
                                            <i class="bi bi-envelope-fill mr-2"></i> Kirim Email Lamaran
                                        </a>
                                    @endif
                                    @if($career->contact_whatsapp)
                                        <a href="https://wa.me/{{ $waNumber }}?text=Halo%20RSIA%20IBI,%20saya%20ingin%20mengirimkan%20lamaran%20pekerjaan%20posisi%20{{ rawurlencode($career->title) }}"
                                            target="_blank" rel="noopener noreferrer" class="btn"
                                            style="width: 100%; justify-content: center; height: 38px; font-size: 0.85rem; border-radius: 8px; background: #25D366; color: white;">
                        ...
                        <a href="{{ route('compro.karir') }}" class="btn"
                            style="width: 100%; justify-content: center; height: 48px; font-size: 0.95rem; border-radius: 10px; margin-top: 12px; background: transparent; color: var(--primary); border: 2px solid var(--primary-soft);">
