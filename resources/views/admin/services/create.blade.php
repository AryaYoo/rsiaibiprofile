...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
    <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="category" required class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all">
                    <option value="medis">Medis & Keperawatan</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Ikon FontAwesome (Opsional)</label>
                <input type="text" name="icon" value="{{ old('icon') }}"
                    class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                    placeholder="Contoh: fas fa-ambulance">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Utama (Opsional)</label>
