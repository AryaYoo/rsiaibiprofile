...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <form action="{{ route('admin.doctors.update', $doctor) }}" method="POST" enctype="multipart/form-data" class="p-8">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                <label for="name" class="block text-sm font-semibold text-gray-700">Nama Lengkap Dokter</label>
                <input type="text" name="name" id="name" value="{{ old('name', $doctor->name) }}" placeholder="Contoh: dr. Nama Dokter, Sp.OG" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" required>
            </div>

            <div class="space-y-2">
                <label for="specialty" class="block text-sm font-semibold text-gray-700">Spesialisasi</label>
                <input type="text" name="specialty" id="specialty" value="{{ old('specialty', $doctor->specialty) }}" placeholder="Contoh: Poli Kandungan" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" required>
            </div>
            ...
            <div class="space-y-2">
                ...
                @if($doctor->image)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $doctor->image) }}" alt="{{ $doctor->name }}" class="w-24 h-24 rounded-lg object-cover border border-gray-200 shadow-sm">
            ...
            <div class="space-y-2">
                ...
                <select name="is_active" id="is_active" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                    <option value="1" {{ old('is_active', $doctor->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('is_active', $doctor->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            </div>
        </div>
        ...
        <div class="mt-8 flex justify-end">
            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg font-bold transition-all flex items-center shadow-md">
                <i class="fas fa-save mr-2"></i> Perbarui Data Dokter
            </button>
        </div>
    </form>
</div>
